...
class ImageProcessor:
    ...
    def __init__(self, port=config.DEFAULT_PORT):
        # Add this new line to initialize stored_images
        self.stored_images = {'38': 0, '39': 0}
    ...
    def run(self):
        ...
        while self.stored_images['38'] + self.stored_images['39'] < 2:
            ...
            if iterations == 0:
                print("[PC] Received dummy frame. Skipping processing but sending reply...")
                self.image_hub.send_reply(b'dummy_ack')
                iterations += 1
                self.stored_images = {'38': 0, '39': 0}
                time.sleep(0.5)
                continue
            if iterations == 1:
                print('Iterations is one here.')
            ...
            if image is not None:
                ...
                if img_path:
                    ...
                    if max_bbox is not None:
                        ...
                        cv2.rectangle(
                            ...
                            (0, 255, 0),
                            -1,
                        )  # Green rectangle for text backg dround
                        cv2.putText(
                            predicted_img,
                            label,
                            (x1, y1 - 5),
                ...
                if image_id in ['38', '39'] :
                    ...
                    # Copy the predicted image to the "Task2" directory
                    shutil.copy(img_path, os.path.join(destination_path, f"image{image_id}_{self.stored_images[image_id]}.jpg"))
                    
                    self.stored_images[image_id] += 1
