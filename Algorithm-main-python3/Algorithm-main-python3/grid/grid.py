...
class Grid:
        ...
        return any(
            [
                (
                    ...
                    and abs(obstacle.position.y - y) < min_separation + 1
                )
                or (
                    obstacle.position.y == y
                    and abs(obstacle.position.x - x) < min_separation + 1
                )
    ...
    def check_valid_position(self, pos: Position, yolo=False):
        ...
        if any(
            obstacle.check_within_boundary(pos, yolo) for obstacle in self.obstacles
        ):
            return False
